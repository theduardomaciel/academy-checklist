...
export function PageMain({ children, className }: { children: ReactNode; className?: string }) {
  return <main className={cn('mx-auto w-full max-w-[640px] flex-1 px-4 pt-5 pb-24', className)}>{children}</main>
}
...
export function PageSubtitle({ children }: { children: ReactNode }) {
  return <p className="mt-0 mb-5 text-sm text-muted-foreground">{children}</p>
}

/** Error banner shown above content. */
export function FormError({ children }: { children: ReactNode }) {
  return (
    <div role="alert" className="mb-3.5 rounded-md bg-destructive/10 px-3 py-2.5 text-[13px] text-destructive">
      {children}
    </div>
  )
}
...
export function Notice({ children }: { children: ReactNode }) {
  return <div className="mb-3 rounded-lg border border-success bg-success-bg px-4 py-3 text-sm text-success">{children}</div>
}

export function FormField({ label, htmlFor, children }: { label: string; htmlFor?: string; children: ReactNode }) {
  return (
    <div className="mb-4">
      <label htmlFor={htmlFor} className="mb-1.5 block text-[13px] font-semibold text-muted-foreground">
        {label}
      </label>
      {children}
    </div>
