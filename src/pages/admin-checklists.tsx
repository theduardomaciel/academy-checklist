...
const emptyItemForm: ItemFormState = { title: '', instructions: '', requiresPhoto: false }
...
export default function AdminChecklists() {
	...
	async function handleCreateTemplate(e: React.FormEvent) {
		...
		if (insertError) {
			...
			setName('')
			setDescription('')
			toast.success(`Checklist "${template.name}" criado. Adicione os itens abaixo.`)
		}
		setCreating(false)
	}
	...
	async function toggleActive(template: TemplateWithItems) {
		...
		toast.success(`Checklist "${template.name}" ${template.is_active ? 'desativado' : 'ativado'}.`)
	}
	...
	return (
		<PageMain>
				...
				templates.map((t) => (
					<Card key={t.id} className="mb-3 shadow-sm">
						<CardContent className="gap-2">
							...
							<div>
								<Button variant="outline" size="sm" onClick={() => setItemTemplateId(t.id)}>
									Adicionar item
								</Button>
							</div>
						</CardContent>
			...
			<Dialog open={itemTemplateId !== null} onOpenChange={(open) => !open && closeItemDialog()}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Adicionar item</DialogTitle>
						<DialogDescription>
							{templates.find((t) => t.id === itemTemplateId)?.name}
						</DialogDescription>
					</DialogHeader>
					<form className="grid gap-4" onSubmit={handleAddItem}>
						<FormField label={`Título do item ${nextOrderFor(itemTemplateId, templates)}`}>
							<Input
								type="text"
						...
						<div className="flex items-center justify-between rounded-md border border-border px-3 py-2.5">
							<Label htmlFor="item-requires-photo">Este item exige foto?</Label>
							<Switch
								id="item-requires-photo"
								checked={itemForm.requiresPhoto}
								onCheckedChange={(checked) => setItemForm((f) => ({ ...f, requiresPhoto: checked }))}
							/>
						</div>
						<DialogFooter>
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							O checklist "{deleteTarget?.name}" e todos os seus itens serão excluídos
